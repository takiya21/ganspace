...
if __name__ == '__main__':
    ...
    outdir_graph = outdir/model.name/layer_key.lower()/est_id/'graph'
    makedirs(outdir_comp, exist_ok=True)
    makedirs(outdir_inst, exist_ok=True)
    makedirs(outdir_summ, exist_ok=True)
    makedirs(outdir_graph, exist_ok=True)

    # Load or compute components
    dump_name = get_or_compute(args, inst, force_recompute=True)
    print(dump_name)
    ...
    # 寄与率のプロット
    print("plot contribution rate")
    cumulative_contribution_rate = [0] + list( np.cumsum(X_var_ratio))
    plt.plot(cumulative_contribution_rate, "-o")
    plt.xlabel("Number of principal components")
    plt.ylabel("Cumulative contribution rate")
    plt.grid()
    plt.show()
    plt.savefig(outdir_graph / 'contribution rate.png')


    with open(outdir_graph / 'explained_variance_ratio.txt', mode="w", newline="") as file:
        for line in cumulative_contribution_rate:
            file.write(str(line) + "\n")
        ...
        for sigma in [args.sigma, 3*args.sigma]:
            for c in range(components):
                for edit_mode in edit_modes:
                    ...
                    make_gif(frames, 5, outdir_comp / f'{get_edit_name(edit_mode)}_sigma{sigma}_comp{c}.gif')
    ...
    n_random_imgs = 5
    #######################  変える  ###############################
    latents = model.sample_latent(n_samples=n_random_imgs) # 画像の始点がランダム
    ####################################################
    #latents = X_global_mean # # 画像の始点を平均
            ...
            for sigma in [args.sigma, 3*args.sigma]: #[2, 5]:
                for edit_mode in edit_modes:
                    ...
                    for c in range(components):
                        ...
                        frames = frames + frames[::-1]
                        make_gif(frames, 5, outdir_inst / f'{get_edit_name(edit_mode)}_sigma{sigma}_img{img_idx}_comp{c}.gif')
