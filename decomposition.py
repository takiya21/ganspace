...
def compute(config, dump_name, instrumented_model):
    ...
    print(f"component: {config.components}")
    ...
    # マッピングネットワークにより、ZからWに変換する
    with torch.no_grad():
        for i in trange(n_lat // B, desc='Sampling latents'):
            # i=0のとき、0~10000までのw空間を生成する.以降も同様に10000ずつ増えていく
            latents[i*B:(i+1)*B] = model.sample_latent(n_samples=B).cpu().numpy()

    ############  中村先輩追加分 (平均と分散確認)  ##############
    # print(latents.shape)
    # latents_mean = latents.mean(axis=0, keepdims=True)
    # latents_std = latents.std(axis=0, keepdims=True)
    # #print(latents_mean)
    # #print(latents_std)
    ...
    # latents = standardization(latents, 0)
    ...
    print(f"samples_are_latents: {samples_are_latents}")
    canceled = False
    print("transformer.batch_support", transformer.batch_support)
    try:
        ...
        for gi in trange(0, N, NB, desc=f'{action} batches (NB={NB})', ascii=True):
            ...
            if transformer.batch_support:
                if not transformer.fit_partial(X.reshape(-1, sample_dims)): # ここでfitする
                    print("transformer.fit_partial(X.reshape(-1, sample_dims)):",transformer.fit_partial(X.reshape(-1, sample_dims)))
                    break
            else:
    ...
    pca_model_path = str(os.path.split(dump_name)[0]) + '/pca_model.pkl'

    with open(pca_model_path, 'wb') as f:
        pickle.dump(transformer.transformer, f)
...
def _compute(submit_config, config, model=None, force_recompute=False):
    ...
    outdir = basedir / f'{config.out_dir}'
